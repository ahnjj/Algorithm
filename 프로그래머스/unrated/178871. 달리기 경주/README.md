...
### ISSUE
- 리스트 원소 순서(hash key) 대로 계속 정렬해야하는 문제
1. index 함수 , 이중 for문 으로 접근시 리스트 원소 수가 많으면 time complexity N x M 으로 너무 커진다.
2. dictionary로 만들어도 value로 key를 찾아야하는 반복작업이 필요하다.

### Approach
- dictionary 2개 value - key / key - value를 만들어둔다.
